...
interface TaskListProps {
  ...
  onRegisterDndList?: (droppableId: string, orderedIds: string[]) => void
}
...
export default function TaskList({
  ...
  const sortedTasks = useMemo(() => sortTasks(tasks, sortKey, sortMode), [tasks, sortKey, sortMode])
  const useDnd = dragMode === 'dnd' && !!dndDroppableId

  useEffect(() => {
    if (!useDnd || !dndDroppableId || !onRegisterDndList) {
      return
    }
    onRegisterDndList(dndDroppableId, sortedTasks.map(task => task.id))
  }, [dndDroppableId, onRegisterDndList, sortedTasks, useDnd])
  ...
  const isNativeDrag = dragMode === 'native'
  ...
  const handleDragStart = (event: DragEvent<HTMLLIElement>, task: Task) => {
    if (!isNativeDrag || !dragEnabled || selectionEnabled) {
      return
    }
  ...
  const canReorder = isNativeDrag && dragEnabled && !selectionEnabled && !!onReorderTasks
  ...
  const renderTaskRow = (
    ...
    const dropIndicatorClass = !useDnd && canReorder && dragOverTaskId === task.id ? 'ring-2 ring-[var(--color-primary-200)]' : ''
    ...
    const dndRef = useDnd && dndProvided ? dndProvided.innerRef : undefined
  ...
  return (
    <>
      ...
      {useDnd ? (
        <Droppable droppableId={dndDroppableId!} type="TASK">
              ...
              {sortedTasks.map((task, index) => {
                ...
                return (
                  <Draggable
                    ...
                    draggableId={task.id}
                    index={index}
                    isDragDisabled={!dragEnabled || selectionEnabled || isEditing}
                  >
